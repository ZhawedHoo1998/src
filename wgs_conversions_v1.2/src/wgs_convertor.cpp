...
std::cout << "datum 获取到数据了么？----" <<  nh_priv.hasParam("datum") <<std:: endl;
    ...
    nh_priv.param("enu_p_odom_x", enu_p_odom_x_, 0.);
    nh_priv.param("enu_p_odom_y", enu_p_odom_y_, 0.);
    nh_priv.param("enu_p_odom_z", enu_p_odom_z_, 0.);
    nh_priv.param("enu_p_odom_yaw", enu_p_odom_yaw_, 0.);
    nh_priv.param("enu_p_odom_pitch", enu_p_odom_pitch_, 0.);
    nh_priv.param("enu_p_odom_roll", enu_p_odom_roll_, 0.);

    nh_priv.param("init_heading", init_heading_, 0.);

    ROS_INFO("init_heading=%lf",init_heading_);

    ROS_INFO("enu->odom tf: enu_p_odom_x_=%lf,enu_p_odom_y=%lf,enu_p_odom_z=%lf,enu_p_odom_yaw=%lf,enu_p_odom_pitch=%lf,enu_p_odom_roll=%lf",
    enu_p_odom_x_,enu_p_odom_y_,enu_p_odom_z_,enu_p_odom_yaw_,enu_p_odom_pitch_,enu_p_odom_roll_);

    tf.transform.translation.x = enu_p_odom_x_;
    tf.transform.translation.y = enu_p_odom_y_;
    tf.transform.translation.z = enu_p_odom_z_;
    double enu_p_odom_yaw_temp = normalize_angle(M_PI-init_heading_);
    // tf.transform.rotation  = tf::createQuaternionMsgFromRollPitchYaw(enu_p_odom_roll_,enu_p_odom_pitch_,enu_p_odom_yaw_);
    tf.transform.rotation = tf::createQuaternionMsgFromYaw(enu_p_odom_yaw_temp);


    // "gps/fix" 来自GPS/RTK FIX数据
    gps_sub_ = nh.subscribe("gps", 1, &wgs_convertor::gpsFixCallback,this);
    // "gps/ins" 组合导航数据
    ins_sub_ = nh.subscribe("gps/ins",50, &wgs_convertor::insCallback,this);


    // "heading" 来自GPS/RTK 航向角数据
    heading_sub_ = nh.subscribe("heading", 1, &wgs_convertor::headingCallback,this);
...
void wgs_convertor::gpsFixCallback(const sensor_msgs::NavSatFixConstPtr& msg)
{
    ...
    if (good_gps)
    {
        ...
        // 转成tf2::Transform
        latest_output_transform_.setOrigin(tf2::Vector3(output[0], output[1], output[2]));//设置平移
        latest_output_covariance_.setZero();
        // latest_output_transform_.setRotation();//设置旋转

        

        // latest_output_transform_.setRotation()
        ...
        geometry_msgs::Pose point_in_enu;//点在enu坐标系下的位置
        tf2::toMsg(latest_output_transform_, point_in_enu);
        KDL::Frame F_bl_ld = transformToBaseLink(point_in_enu,tf.transform);//将enu坐标系下的点转换到odom坐标系下

        // tf2::toMsg(latest_output_transform_, gps_pose.pose.pose);
        gps_pose.pose.pose.position.x = F_bl_ld.p.x();
        gps_pose.pose.pose.position.y = F_bl_ld.p.y();
        gps_pose.pose.pose.position.z = F_bl_ld.p.z();
        gps_pose.pose.pose.orientation = tf::createQuaternionMsgFromYaw(yaw_vehicle_);

        // gps_pose.pose.pose.orientation.w = 1;
...
// heading数据回调
void wgs_convertor::headingCallback(const geometry_msgs::QuaternionStampedConstPtr& msg)
{
    yaw_vehicle_  = -normalize_angle((tf::getYaw(msg->quaternion) - init_heading_));
     

    // heading_msg_ = *msg;//获取gps的航向角
...
void wgs_convertor::insCallback(const nav_msgs::OdometryConstPtr &msg)
{
    ...
    ROS_INFO("lon_lat_hei : [%f  %f  %f]",msg->pose.pose.position.y, msg->pose.pose.position.x, msg->pose.pose.position.z);
    ...
    if (good_gps)
    {
        ...
        double yaw=tf::getYaw(ins_odom_convert_result.pose.pose.orientation);
         ROS_INFO("Yaw angle is :[%f ] ",yaw*180/3.14);
...
KDL::Frame wgs_convertor::transformToBaseLink(const geometry_msgs::Pose& pose,
...
出口参数：无
********************************************************/
double wgs_convertor::normalize_angle(double angle)
{
    if (angle>M_PI)
    {
        angle -=2.0*M_PI;
